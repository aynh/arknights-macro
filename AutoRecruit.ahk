...
AutoRecruit() {
  ...
  for combination_idx in combinations {
    ...
    if matches.Length > 0 {
      matches_map.Set(ArrayJoin(combination_idx, '|'), matches)
    }
  }

  best_combination := []
  min_rarity := AutoRecruitConst.MIN_RARITY
  for combination, matches in matches_map {
    combination_min_rarity := AutoRecruitConst.MAX_RARITY
    for operator in matches
      if operator.rarity < combination_min_rarity
        combination_min_rarity := operator.rarity

    combination_arr := StrSplit(combination, '|')
    if (
      ; replace the last best combination if the current rarity is higher
  ...
  SetMouseDelay(80)
  if min_rarity >= 4 {
    ; click the tags and set timer to max if minimum rarity is equal or greater than 4
    ...
    ; otherwise, set timer to 7:40:00 for getting at least 3★
    loop 3
